...
import { createSeededProduct, createSeededUser, ensureOpenCash, fetchCurrentCash, fetchProductStock } from './integration-helpers'
...
test('PDV integrado registra pagamento misto com troco e reflete so o dinheiro liquido no caixa', async ({ page, request }) => {
  const suffix = `${Date.now()}`
  const user = await createSeededUser(request, suffix)
  const product = await createSeededProduct(request, user.token, suffix)
  const initialCash = await ensureOpenCash(request, user.token)
  ...
  await productsSection.getByRole('button', { name: new RegExp(product.nome) }).click()

  await page.getByLabel('Valor pagamento 1').fill('10')
  await page.getByLabel('Recebido pagamento 1').fill('15')
  await page.getByRole('button', { name: 'Adicionar pagamento' }).click()
  await page.getByLabel('Forma pagamento 2').selectOption('4')
  await page.getByLabel('Valor pagamento 2').fill('15')

  await page.getByRole('button', { name: 'Finalizar Venda' }).click()

  await expect(page.getByRole('heading', { name: /Venda conclu/i })).toBeVisible()
  await expect(page.getByText('Troco:')).toBeVisible()

  await expect.poll(async () => fetchProductStock(request, user.token, product.id)).toBe(4)
  await expect.poll(async () => {
    const cash = await fetchCurrentCash(request, user.token)
    return cash?.valor_em_dinheiro_vendas ?? 0
  }).toBe((initialCash.valor_em_dinheiro_vendas ?? 0) + 10)
  await expect.poll(async () => {
    const cash = await fetchCurrentCash(request, user.token)
    return cash?.saldo_esperado ?? 0
  }).toBe((initialCash.saldo_esperado ?? 0) + 10)
})
